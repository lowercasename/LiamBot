...
const generate = ({ className, raceName, primaryStat, secondaryStat, level }) => {
  let character = {};
  let classType;
  if (!className || typeof className !== 'string') {
    classType = classes[Math.floor(Math.random() * classes.length)];
  } else {
    let needle = className.toLowerCase().substring(0, 4);
    classType = classes.find(o => o.name.toLowerCase().startsWith(needle));
    if (!classType) {
      classType = classes[Math.floor(Math.random() * classes.length)];
    }
  }
  ...
  if (!raceName || typeof raceName !== 'string') {
    race = races[Math.floor(Math.random() * classes.length)];
  } else {
    let needle = raceName.toLowerCase();
    race = races.filter(o => o.name.toLowerCase().includes(needle) || o.type.toLowerCase().includes(needle));
    if (!race) {
      race = races[Math.floor(Math.random() * classes.length)];
    }
    if (race.length > 1) {
  ...
  if (!secondaryStat  || typeof secondaryStat !== 'string' || !abilities.includes(secondaryStat.toUpperCase())) {
    secondaryStat = classType.secondary;
  } else {
  ...
  .map((i, index) => {
    ...
    let matchingModifier = Object.entries(race.modifiers).find(a => a[0] === statName);
    if (matchingModifier) {
  ...
  character.class = classType.name;
  character.race = race.name;
  character.level = level;
  character.abilities = scores;
  character.proficiencyBonus = '+' + Math.ceil(level / 4 + 1);
  character.hp = classType.hd + parseInt(character.abilities.find(o => o.name === 'CON').modifier) + (level > 1 ? (Math.ceil(classType.hd / 2) + parseInt(character.abilities.find(o => o.name === 'CON').modifier)) * (level - 1) : 0);
  character.notes = ("" + (race.notes || "") + (level >= 4 ? ` For levelling, increase one Ability Score by 2 or two Ability Scores by 1${level >= 8 ? `, ${Math.floor(level / 4)} times.` : '.' }` : '')).trim();

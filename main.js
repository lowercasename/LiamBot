...
client.on('message', async message => {
    ...
    if (args.includes('save')) {
      if (message.reference) {
        ...
        return message.channel.send(`To save a new quote, reply to the message you want to save and and type **${prefix}quote save**.`);
      }
    } else {
      ...
      db.query(`SELECT * FROM quotes WHERE server = ? ORDER BY RAND() LIMIT 1;`, [serverId], function (error, results, fields) {
        ...
        if (result) {
          return message.channel.send(`**@${result.quote_author_username}** ${result.quote}`);
        } else {
